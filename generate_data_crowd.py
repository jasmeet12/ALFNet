...
annos = []
with open(res_path1, 'rb') as config_dictionary_file:
    annos = pickle.load(config_dictionary_file)

for type in types:
    anno_path = os.path.join(all_anno_path, 'anno_' + type + '.mat')
    res_path = os.path.join('data/cache/crowdHuman', type)
    image_data = []
    ...
    for l in range(len(annos)):
        ...
        # image_data = image_data[0:100]

	with open(res_path, 'wb') as fid:
		cPickle.dump(image_data, fid, cPickle.HIGHEST_PROTOCOL)
	print '{} has {} images and {} valid images, {} valid gt and {} ignored gt'.format(type, len(annos), valid_count, box_count, iggt_count)